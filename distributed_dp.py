...
def run_backend(backend, world_size=2):
    ...
    if backend == "jax":
        ...
        if num_gpus < world_size:
            os.environ["XLA_FLAGS"] = f"--xla_force_host_platform_device_count={world_size} --xla_cpu_multi_thread_eigen=false"
            os.environ["OMP_NUM_THREADS"] = str(world_size)
            os.environ["MKL_NUM_THREADS"] = str(world_size)
            os.environ["OPENBLAS_NUM_THREADS"] = str(world_size)
