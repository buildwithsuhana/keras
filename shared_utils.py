...
def get_layout_map(mesh):
    ...
    layout_map[".*self_attention/(query|key|value)/kernel"] = (
        None,
        "model",
        None,
    )
...
def train_model(distribution, x, y, epochs=10, steps=5, batch_size=4):
    with distribution.scope():
        ...
        model.compile(
            optimizer=keras.optimizers.Adam(1e-5), loss="mse", jit_compile=False
        )
        ...
        history = model.fit(
            dataset, epochs=epochs, steps_per_epoch=steps, verbose=1
        )
