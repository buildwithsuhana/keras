...
def _safe_barrier(timeout_seconds=60):
    ...
    try:
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            ...
            if world_size > 1:
                # Use a simple timeout-based approach instead of full barrier
                start_time = time.time()

                # Try all_reduce with timeout
                try:
...
def run_opt_hybrid_dp_mp_test():
    ...
    # CRITICAL: Build the model explicitly before training
    # This prevents "model not built" errors when calling fit()
    print(f"\n[Rank {local_rank}] Building model explicitly...")
    try:
        # IMPORTANT: Disable DTensor distribution during model building
        # to avoid NCCL collective operations that can cause hangs
        # when weights have replicated DTensor layouts
        os.environ["KERAS_DISTRIBUTION_DISABLE"] = "1"

        # Build with a dummy input to ensure all variables are created
        if hasattr(model, 'sampler'):
            # For OPT model, use token_ids input
            dummy_token_ids = np.zeros((1, 4), dtype=np.int32)
            dummy_padding_mask = np.ones((1, 4), dtype=np.int32)
            model({
                "token_ids": dummy_token_ids,
                "padding_mask": dummy_padding_mask
            })
            print(f"[Rank {local_rank}] ✓ OPT model built with dummy input")
        else:
            # For simple dense model
            dummy_input = np.zeros((1, 64), dtype=np.float32)
            model(dummy_input)
            print(f"[Rank {local_rank}] ✓ Simple model built with dummy input")

        # Restore DTensor distribution for actual training/inference
        os.environ.pop("KERAS_DISTRIBUTION_DISABLE", None)
        ...
        print(f"[Rank {local_rank}] ✓ Model compiled")

    except Exception as e:
        # Make sure to restore the flag even on error
        os.environ.pop("KERAS_DISTRIBUTION_DISABLE", None)
        print(f"[Rank {local_rank}] Warning: Could not build/compile model: {e}")
        import traceback
        traceback.print_exc()
        # Continue anyway - model might be used for inference only
    ...
    # Sync all ranks before starting forward pass
    # This ensures all ranks are ready before distributed operations
    print(f"\n{'='*70}")
    print(f"TEST: FORWARD PASS")
    print(f"{'='*70}")

    # Barrier to ensure all ranks are synchronized
    print(f"[Rank {local_rank}] Synchronizing before forward pass...")
    _safe_barrier()
