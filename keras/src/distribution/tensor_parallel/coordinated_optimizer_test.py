...
@pytest.mark.skipif(
    backend.backend() != "jax",
    reason="Tensor Parallelism features are currently optimized for the JAX backend.",
)
class TensorParallelOptimizerTest(testing.TestCase):
    ...
    def test_initialization(self):
        """Verifies optimizer initializes with correct base optimizer and defaults."""
    ...
    def test_init_from_string(self):
        """Verifies optimizer correctly fetches base optimizer from string ID."""
    ...
    def test_build_and_state_sharding(self):
        """Verifies that building the optimizer partitions state variables correctly."""
        device_count = 4
        optimizer = TensorParallelOptimizer(optimizers.Adam(), device_count=device_count)
        ...
        self.assertEqual(len(sharded_states["momentum"][dense_1_kernel_path]), device_count)

    def test_apply_gradients_fallback(self):
        """Verifies fallback to standard update logic when grads are not sharded."""
        device_count = 2
    ...
    def test_synchronize_gradients_logic(self):
        ...
        device_count = 2
        model = self._get_simple_model()
        optimizer = TensorParallelOptimizer(optimizers.SGD(), device_count=device_count)
