...
class TensorParallelKeras(keras.Model):
    ...
    def _validate_sharding(self):
        """
        Compares the autoconfig plan to the actual sharded weights.
        This is a powerful debugger for mismatch bugs.
        """
        if not hasattr(self, 'tensor_parallel_config') or not self.tensor_parallel_config:
            logger.warning("Validation skipped: no tensor_parallel_config found.")
            return

        if not hasattr(self, 'model_shards') or not self.model_shards:
            logger.warning("Validation skipped: no model_shards found.")
            return

        logger.info("🕵️  Validating sharding plan...")
        config_rules = set(self.tensor_parallel_config.state_rules.keys())
        
        # Get the keys from the *actual* sharded weights on shard 0
        try:
            sharded_weights = set(
                self.model_shards[0].sharding_strategy.sharded_weights.keys()
            )
        except Exception as e:
            logger.error(f"Failed to get sharded weights from shard 0: {e}")
            return
            
        if config_rules == sharded_weights:
            logger.info("✅ SHARDING PLAN VERIFICATION PASSED: Config and Sharder are in sync.")
            return

        logger.warning("⚠️  SHARDING MISMATCH DETECTED!")
        
        missed_by_sharder = config_rules - sharded_weights
        if missed_by_sharder:
            logger.warning(
                f"   [CONFIG ERROR] {len(missed_by_sharder)} rules from autoconfig "
                f"were NOT applied by the sharder (mismatch?): {missed_by_sharder}"
            )
            
        extra_by_sharder = sharded_weights - config_rules
        if extra_by_sharder:
             logger.warning(
                f"   [SHARDER ERROR] {len(extra_by_sharder)} weights were sharded "
                f"that were NOT in the autoconfig plan: {extra_by_sharder}"
            )
