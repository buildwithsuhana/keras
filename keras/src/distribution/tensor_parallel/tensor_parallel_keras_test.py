...
class TensorParallelKerasTest(testing.TestCase):
    ...
    def test_initialization_and_sharding_verification(self):
        ...
        if self.can_run_distributed:
            ...
            self.assertLess(
                shard_0_params,
                original_params,
                "Sharding should reduce parameters on a single shard.",
            )
        else:
        ...
        self.assertEqual(
            tp_model_total_params, self.original_model.count_params()
        )
    ...
    def test_distributed_training_workflow(self):
        ...
        if not self.can_run_distributed:
            ...
            self.skipTest(
                f"Skipping distributed test because device_count={self.device_count} (must be > 1)"
            )
        ...
        self.assertTrue(
            hasattr(tp_model, "coordinated_optimizer"),
            "Coordinated optimizer should be created in distributed mode.",
        )
