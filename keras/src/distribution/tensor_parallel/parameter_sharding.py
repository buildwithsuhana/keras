...
class ParameterShardingStrategy:
    ...
    def shard_model_parameters(self, shard_model, weight_loader, config, device_id):
        ...
        # DEBUGGER: Track processed variables
        print(f"   [DEBUG] Sharding Strategy starting for {device_id}. Total Variables: {len(shard_model.variables)}")
        
        for target_var in shard_model.variables:
            name = target_var.path
            # Match using the forward-slash paths generated by Fixed AutoConfig
            ...
            if id(target_var) in var_to_owner:
                ...
                new_v = self._replace_variable(layer, attr_name, target_var, sliced_val_tensor, device_id)
                owner_found = True
                print(f"      [DEBUG] Replaced '{name}' via Owner Map on {device_id}")
            else:
                ...
                try:
                    ...
                    owner_found = True
                    print(f"      [DEBUG] Replaced '{name}' via Path Injection on {device_id}")
                except Exception:
                    pass

            if not owner_found:
                # Final fallback for variables not bound to attributes (forces physical move)
                print(f"      [WARNING] No owner found for '{name}'. Using direct assign.")
                with keras.device(device_id):
