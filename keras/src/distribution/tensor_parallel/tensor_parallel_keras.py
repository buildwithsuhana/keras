...
class TensorParallelKeras(Model):
    def __init__(
        ...
        **kwargs,
        ...
        for rank, device_id in enumerate(self.devices):
            ...
            # --- CRITICAL FIX: Create shard ON TARGET DEVICE ---
            # Previously 'cpu' was used here, causing the variables to get stuck on CPU.
            # By using device_id, we force allocation on the specific TPU/GPU.
            ...
            # Load and assign weights (now happening on the correct device)
    ...
    def compile(self, optimizer=None, **kwargs):
        if len(self.model_shards) > 1 and optimizer:
            ...
            # Important: Compile each shard with its own optimizer instance
            # This ensures optimizer variables are created on the shard's device
            for i, shard in enumerate(self.model_shards):
