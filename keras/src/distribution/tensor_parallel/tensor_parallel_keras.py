...
class TensorParallelKeras(models.Model):
    def __init__(self, model, device_count=None, device_ids=None, rank=0, **kwargs):
        ...
        if device_ids is None:
            ...
            if not device_ids: device_ids = [d for d in all_devices if "gpu" in d.lower()]
            if not device_ids: device_ids = [d for d in all_devices if "cpu" in d.lower()]
        ...
        print(f"✅ [TP Class] Initialized. Rank: {self.rank}, Device: {self.current_device}")
