...
train_dataset = dataset.map(split_input_target).batch(BATCH_SIZE, drop_remainder=True)
...
def create_model():
    ...
    opt_model.backbone.token_embedding.embeddings_initializer = keras.initializers.RandomNormal(stddev=0.02)
...
device_mesh = DeviceMesh(shape=(len(DEVICES),), axis_names=("model",), devices=DEVICES)
...
plt.plot(distributed_history.history["loss"], label="Distributed Training Loss (2 Virtual CPUs)", marker='o')
plt.plot(single_device_history.history["loss"], label="Single Device Loss (Baseline)", marker='x', linestyle='--')
