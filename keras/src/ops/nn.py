...
def dot_product_attention(
    ...
    if hasattr(query, "device_mesh") or hasattr(key, "device_mesh") or hasattr(value, "device_mesh"):
        if scale is None:
        ...
        logits = backend.numpy.matmul(q, backend.numpy.transpose(k, (0, 1, 3, 2))) / scale
        ...
        if hasattr(out, "device_mesh") and hasattr(out, "placements"):
            ...
            if any(isinstance(p, Shard) for p in out.placements):
                out = out.redistribute(out.device_mesh, [Replicate()] * out.device_mesh.ndim)
        return out
