...
class TorchTrainer(base_trainer.Trainer):
    ...
    def _initialize_ddp(self):
        if torch.distributed.is_initialized() and not hasattr(
            ...
            if (
                active_distribution is None
                or isinstance(active_distribution, DataParallel)
                or isinstance(active_distribution, FSDP)
            ):
                device = get_device()
                ...
                if active_distribution is not None:
                    backend_mesh = active_distribution.device_mesh.backend_mesh
                ...
                object.__setattr__(self, "ddp_model", wrapper)
        ...
        if torch.distributed.is_initialized():
            with torch.no_grad():
                ...
                if all_vars:
                    ...
                    if active_distribution is not None:
                        backend_mesh = (
                            active_distribution.device_mesh.backend_mesh
                        )
                        process_group = backend_mesh.get_group(
                            active_distribution.batch_dim_name
                        )
