...
class TorchCheckpointTest(TorchDistributedTestCase):
    ...
    def _checkpoint_test(self, rank, world_size):
        ...
        with dist.scope():
            ...
            weights = [np.ones((8, world_size * 4)), np.zeros((world_size * 4,))]
    ...
    def _unbind_test(self, rank, world_size):
        ...
        with dist.scope():
            ...
            for t in unbound:
                ...
                self.assertTrue(all(isinstance(p, Replicate) for p in t.placements))
...
class TorchVariableDistributionTest(TorchDistributedTestCase):
    ...
    def _non_mp_variable_test(self, rank, world_size):
        # Test that variables are NOT moved to local rank if ModelParallel is not active
        v = backend.Variable(torch.ones(2, 2))
        self.assertFalse(hasattr(v.value, "device_mesh"))
        # device type might be mps or cpu depending on env, but it should not be forced to cuda
        self.assertNotEqual(v.value.device.type, "cuda")

    def test_non_mp_variable(self):
        self.run_distributed(TorchVariableDistributionTest._non_mp_variable_test)
...
class TorchPyDatasetAdapterTest(TorchDistributedTestCase):
    ...
    def test_py_dataset_sharding(self):
        self.run_distributed(TorchPyDatasetAdapterTest._py_dataset_sharding_test)
...
class TorchDataLoaderShuffleTest(TorchDistributedTestCase):
    ...
    def test_dataloader_shuffle(self):
        self.run_distributed(TorchDataLoaderShuffleTest._dataloader_shuffle_test)
...
class TorchDistributionTensorTest(TorchDistributedTestCase):
    ...
    def _distribute_data_input_dp_test(self, rank, world_size):
        ...
        with dist.scope():
            # In DP, distribute_data_input in backend is only called for ModelParallel
            # but we can test the behavior of the backend function directly
    ...
    def _convert_to_tensor_sharding_test(self, rank, world_size):
        ...
        with dist.scope():
            # Plain tensor converted to tensor should be sharded if ModelParallel is active
            # even if it's just a constant conversion
            t = backend.convert_to_tensor(np.ones((world_size * 2, 4), dtype="float32"))
            self.assertIsInstance(t, DTensor)
            self.assertEqual(t.shape, (world_size * 2, 4))

    def test_distribute_data_input_dp(self):
        self.run_distributed(TorchDistributionTensorTest._distribute_data_input_dp_test)

    def test_convert_to_tensor_sharding(self):
        self.run_distributed(TorchDistributionTensorTest._convert_to_tensor_sharding_test)
...
class TorchTrainerDistributionTest(TorchDistributedTestCase):
    ...
    def _distribute_inputs_test(self, rank, world_size):
        ...
        x = torch.randn(4, 8)
        # For DataParallel, _distribute_inputs currently doesn't shard if not ModelParallel
        # and not replicate=True. 
        with dist.scope():
            distributed_x = trainer._distribute_inputs(dist, x)
            self.assertIs(distributed_x, x)
        
        # Test ModelParallel sharding
        mesh_mp = distribution_lib.DeviceMesh((1, world_size), ["batch", "model"])
    ...
    def test_distribute_inputs(self):
        self.run_distributed(TorchTrainerDistributionTest._distribute_inputs_test)
