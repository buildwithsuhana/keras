...
def sharding_scope():
    ...
    try:
        ...
        global_state.set_global_attribute("enable_torch_sharding", False)
...
def all_reduce(tensor, op="sum"):
    ...
    if op.lower() == "sum":
        ...
        # PyTorch doesn't have MEAN reduce op in all_reduce directly,
        # usually SUM then divide by world size
        reduce_op = torch.distributed.ReduceOp.SUM
    else:
