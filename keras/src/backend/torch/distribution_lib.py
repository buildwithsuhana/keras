...
def _get_device_type(device_type=None):
    """Resolve the device type to a backend-specific string.

    Args:
        device_type: Optional string, one of "cpu", "gpu", "cuda", or "tpu".
            If None, it probes the system for available hardware.

    Returns:
        A string, one of "cpu", "cuda", or "tpu".
    """
    if not device_type:
        device_type = os.environ.get("KERAS_TORCH_DEVICE")

    if device_type:
        device_type = device_type.lower()
        return "cuda" if device_type == "gpu" else device_type

    if xm and xm.xla_device_count() > 0:
        return "tpu"
...
def list_devices(device_type=None):
    ...
    """
    resolved_device_type = _get_device_type(device_type)
...
def get_device_count(device_type=None):
    ...
    if torch.distributed.is_initialized() or "WORLD_SIZE" in os.environ:
        actual_device_type = _get_device_type()
    ...
    if resolved_device_type == "tpu" and xm:
...
def _to_backend_mesh(device_mesh):
    ...
    resolved_device_type = _get_device_type(first_device)
...
def _to_backend_device(device_name):
    ...
    resolved_device_type = _get_device_type(device_type_str)
