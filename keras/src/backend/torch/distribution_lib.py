...
def list_devices(device_type=None):
    """Essential for DeviceMesh initialization."""
    if device_type:
    ...
    if device_type in ("gpu", "cuda"):
        return [f"cuda:{i}" for i in range(torch.cuda.device_count())] if torch.cuda.is_available() else []
    return ["cpu:0"] if device_type == "cpu" else []

def distribute_variable(value, layout):
    """Required by the Keras Variable constructor; now a simple pass-through."""
    return value

def initialize(job_addresses, num_processes, process_id):
    """Required to set up the NCCL/Gloo communication group."""
...
def initialize_rng():
    """Ensures deterministic weight init/dropout across sharded models."""
    global_seed = rng_utils.get_random_seed()
    if global_seed is None:
        if not dist.is_initialized():
            seed = seed_generator.make_default_seed()
        else:
            seed_tensor = torch.tensor(seed_generator.make_default_seed(), dtype=torch.int64, device="cpu") if process_id() == 0 else torch.empty(1, dtype=torch.int64, device="cpu")
...
def all_reduce(x, op="sum", axis_name="model"):
    """Used by 'assembled_model' to merge results from Row-Parallel shards."""
...
def all_gather(x, axis, axis_name="model"):
    """Used by 'assembled_model' to merge results from Column-Parallel shards."""
    if not dist.is_initialized(): return x
