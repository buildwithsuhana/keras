...
def activate_dtensor_promotion():
    ...
    global _DTENSOR_PROMOTION_ACTIVE
    if (
        _GLOBAL_DTENSOR_PROMOTION_MODE is not None
        and not _DTENSOR_PROMOTION_ACTIVE
    ):
...
def deactivate_dtensor_promotion():
    global _GLOBAL_DTENSOR_PROMOTION_MODE
    global _DTENSOR_PROMOTION_ACTIVE
    if _GLOBAL_DTENSOR_PROMOTION_MODE is not None and _DTENSOR_PROMOTION_ACTIVE:
...
class KerasDTensorPromotionMode(torch.utils._python_dispatch.TorchDispatchMode):
    """Torch dispatch mode that promotes mixed Tensor/DTensor arguments.
...
def _promote_tensor_args(args, kwargs):
    from torch.distributed.tensor import DTensor
    from torch.distributed.tensor import Replicate

    flat_args = tree.flatten((args, kwargs))
    dtensor_arg = next(
        (x for x in flat_args if isinstance(x, DTensor)),
        None,
    )
    ...
    return tree.map_structure(promote, args), tree.map_structure(
        promote, kwargs
    )
...
def compute_output_spec(fn, *args, **kwargs):
    ...
    with StatelessScope(), SymbolicScope(), torch.no_grad():
        deactivate_dtensor_promotion()
        try:
            outputs = symbolic_call(fn, args, kwargs, fill_value=83)
        finally:
            activate_dtensor_promotion()
