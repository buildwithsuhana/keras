...
class TestJaxDistributedFunctions(testing.TestCase):
    ...
    def test_broadcast_distributes_from_root_in_pmap(self):
        """Tests that 'broadcast' correctly sends data from root to all devices."""
        x_broadcast = ops.array([5.0, 6.0])
        sharded_broadcast_input = jnp.stack(
            [x_broadcast] + [jnp.zeros_like(x_broadcast)] * (self.world_size - 1)
        )
    ...
    def test_scatter_distributes_chunks_in_pmap(self):
        """Tests that 'scatter' correctly distributes chunks from the root device."""
        ...
        pmapped_scatter = jax.pmap(
            lambda x: self.comm_ops["scatter"](x, root=0, axis=0), axis_name="data"
        )
