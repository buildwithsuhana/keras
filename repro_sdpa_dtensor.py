...
def simple_dot_product_attention(
    ...
    if is_dtensor:
        ...
        out_l = _scaled_dot_product_attention(
...
def test_dtensor_sdpa():
    ...
    dq = distribute_tensor(q.detach(), device_mesh, [layout]).requires_grad_(
        True
    )
    dk = distribute_tensor(k.detach(), device_mesh, [layout]).requires_grad_(
        True
    )
    dv = distribute_tensor(v.detach(), device_mesh, [layout]).requires_grad_(
        True
    )
