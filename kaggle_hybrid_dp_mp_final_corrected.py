"""Test hybrid Data Parallel + Model Parallel with corrected DTensor detection.

This version properly checks for DTensor by accessing .data attribute.
"""
...
def run_hybrid_dp_mp_test():
    """Run the hybrid DP+MP test with proper DTensor detection."""
    ...
    # Create LayoutMap with correct patterns (using exact path patterns)
    layout_map = LayoutMap(mesh)
    
    # Use exact path matching patterns
    ...
    layout_map[".*feedforward.*intermediate.*kernel"] = (None, "model")
    layout_map[".*feedforward.*output.*kernel"] = ("model", None)
    layout_map[".*pooled_dense.*kernel"] = (None, "model")
    layout_map[".*logits.*kernel"] = (None, "model")
    ...
    # Verify sharding - CORRECTED DTensor detection
    print(f"\n{'='*70}")
    ...
    for v in model.variables:
        ...
        # v.value returns torch.nn.Parameter (if it's a parameter)
        # v.value.data returns the actual tensor (DTensor if sharded)
        
        torch_tensor = None
        
        # Check if value is a Parameter with data attribute
        if hasattr(v, 'value') and hasattr(v.value, 'data'):
            torch_tensor = v.value.data
        # Direct check on value
    ...
    if sharded_count > 0:
        print(f"\n[Rank {local_rank}] ✓ MODEL PARALLELISM VERIFIED!")
    else:
        print(f"\n[Rank {local_rank}] ⚠️ No sharded weights detected")
