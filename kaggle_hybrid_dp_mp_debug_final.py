"""Test hybrid Data Parallel + Model Parallel with debug mode enabled.

This version enables KERAS_DISTRIBUTION_DEBUG to verify sharding is applied.
"""
...
# Enable debug mode to see distribution calls
...
def run_hybrid_dp_mp_test():
    """Run the hybrid DP+MP test with debug mode enabled."""
    ...
    # Create LayoutMap with correct patterns
    ...
    # Verify sharding by checking _torch attribute
    print(f"\n{'='*70}")
    ...
    for v in model.variables:
        # Get the underlying torch tensor (may be wrapped in Parameter)
        torch_tensor = v.value if hasattr(v, 'value') else v
        ...
        if isinstance(torch_tensor, DTensor):
            ...
            if isinstance(wrapped, DTensor):
                ...
                global_shape = tuple(torch_tensor.shape)
        elif hasattr(torch_tensor, 'data') and isinstance(torch_tensor.data, DTensor):
            is_dtensor = True
            local_shape = tuple(torch_tensor.data.to_local().shape)
            global_shape = tuple(torch_tensor.shape)
    ...
    if sharded_count > 0:
        print(f"\n[Rank {local_rank}] ✓ SHARDING VERIFIED! Model parallelism is working.")
    else:
        print(f"\n[Rank {local_rank}] ⚠️ No sharded weights detected.")
        print(f"  Check the debug output above to see if distribute_variable() is being called.")
