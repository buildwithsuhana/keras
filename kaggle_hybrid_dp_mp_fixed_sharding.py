"""Test hybrid Data Parallel + Model Parallel with CORRECTED LayoutMap patterns.

This version uses the correct sharding patterns for BERT attention mechanism.
"""
...
def run_hybrid_dp_mp_test():
    """Run the hybrid DP+MP test with corrected sharding patterns."""
    ...
    # 
    # Q/K/V weight shape: (hidden_size, num_heads, head_size) = (128, 2, 64)
    # Output weight shape: (num_heads, head_size, hidden_size) = (2, 64, 128)
    #
    # For proper model parallelism, we should shard the LAST dimension:
    # - Q/K/V: (128, 2, 64) -> shard dim 2 (64) -> shard embedding dim -> layout (None, None, "model")
    #   But BERT-tiny has head_size=64, so we can only shard if we shard dim 1 (heads=2) 
    #
    # For 2 GPUs, let's shard dimension 0 (heads) or keep it replicated:
    # - Better: replicate heads, shard the FFN layers
    #
    # CORRECT patterns for BERT-tiny:
    layout_map = LayoutMap(mesh)
    
    # FFN layers - these are the large layers that benefit from sharding
    layout_map[".*feedforward.*intermediate.*kernel"] = ("model",)
    layout_map[".*feedforward.*output.*kernel"] = ("model",)
    
    # Attention - use careful sharding
    # For BERT-tiny with 2 heads of 64, the Q/K/V kernels are (128, 2, 64)
    # If we shard on dim 1 (heads=2), each GPU gets 1 head - but reshape operations fail
    # So we replicate the attention projections
    ...
    # Embeddings - replicate (can't shard easily due to gather operations)
    layout_map["token_embedding/embeddings"] = ()   # Replicate
    layout_map["position_embedding/embeddings"] = ()  # Replicate
    layout_map["segment_embedding/embeddings"] = ()  # Replicate
    
    # Classifier layers
    layout_map[".*pooled_dense.*kernel"] = ()  # Replicate
    layout_map[".*logits.*kernel"] = ()        # Replicate
    
    print(f"[Rank {local_rank}] LayoutMap patterns configured (FFN sharding only)")
    ...
    print(f"\n[Rank {local_rank}] Checking for sharded DTensor weights...")
    print("-" * 60)
