...
This version uses a proper 2D DeviceMesh with matching placements.
"""
...
def run_hybrid_dp_mp_test():
    """Run the hybrid DP+MP test with proper 2D DeviceMesh."""
    ...
    # Create DeviceMesh with ALL GPUs - use 2D mesh (data, model)
    # For num_gpus=2, we use shape=(1, 2) for pure model parallelism
    devices = [f"cuda:{i}" for i in range(num_gpus)]
    
    # 2D mesh: (data_parallel, model_parallel)
    dp_dim = 1  # No data parallelism, just model parallelism
    mp_dim = num_gpus
