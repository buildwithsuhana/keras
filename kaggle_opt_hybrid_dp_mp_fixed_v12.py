...
FIXED VERSION V12: Fix pattern matching and input tensor DTensor conversion.

KEY FIXES:
1. Fix regex patterns to properly match weight paths with underscores
2. Convert input tensors to DTensors before forward pass when distribution is enabled
3. Ensure proper handling of mixed tensor types in distributed operations
...
def redistribute_model_weights_properly(model, strategy, layout_map):
    ...
    This creates proper DTensors by calling torch_distribute_tensor on each weight,
    which handles the sharding correctly.
    """
    import re
    from torch.distributed._tensor import DTensor, Replicate, Shard
    from torch.distributed.device_mesh import init_device_mesh
...
def run_opt_hybrid_dp_mp_test():
    ...
    print(f"\n{'='*70}")
    print(f"TEST: OPT MODEL WITH HYBRID DATA PARALLEL + MODEL PARALLEL (V12)")
    print(f"{'='*70}")
    ...
    # FIXED PATTERNS V12: Use proper regex patterns that match the actual weight names
    ...
    print(f"[Rank {local_rank}] NOTE: Using '.*self_attention.*' and '.*feedforward.*' patterns (with underscores)")
    ...
    try:
        ...
        if is_opt_model:
            ...
            # CRITICAL V12 FIX: Convert input tensors to DTensors before forward pass
            ...
            if isinstance(model.layers[-1], keras.layers.Dense) and model.output_shape[-1] == 10:
                ...
                # CRITICAL V12 FIX: Convert input to DTensor
                x_tensor = torch.from_numpy(x)
                if torch.cuda.is_available():
                    x_tensor = x_tensor.cuda()
    ...
    try:
        ...
        if is_opt_model:
            ...
            # CRITICAL V12 FIX: Convert training inputs to DTensors
            token_ids_tensor = torch.from_numpy(token_ids).long()
            padding_mask_tensor = torch.from_numpy(padding_mask)
