...
def run_training(rank, world_size, layout_map, backend):
    ...
    with distribution.scope():
        ...
        if backend == "torch":
            # Per-process batch size for Torch
            batch_size = global_batch_size // world_size
            
            # Simple data sharding for Torch
            full_token_ids = np.random.randint(0, 50272, (global_batch_size * 10, 32)).astype("int32")
            full_padding_mask = np.ones((global_batch_size * 10, 32), dtype="int32")
            full_y = np.random.normal(size=(global_batch_size * 10, 32, 768)).astype("float32")
            
            # Manually shard data across ranks
            indices = np.arange(rank * batch_size, (rank + 1) * batch_size)
            # Add indices for subsequent steps
            for i in range(1, 10):
                indices = np.concatenate([indices, np.arange(i * global_batch_size + rank * batch_size, i * global_batch_size + (rank + 1) * batch_size)])
            ...
            # Global batch size for JAX
            batch_size = global_batch_size
            x = {
                "token_ids": np.random.randint(0, 50272, (global_batch_size * 10, 32)).astype("int32"),
                "padding_mask": np.ones((global_batch_size * 10, 32), dtype="int32")
            }
            y = np.random.normal(size=(global_batch_size * 10, 32, 768)).astype("float32")

        # Warmup
        model.fit({k: v[:batch_size] for k, v in x.items()}, 
                  y[:batch_size], 
