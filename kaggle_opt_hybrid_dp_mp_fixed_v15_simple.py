...
KEY FIXES:
1. Rely on distribution_lib.initialize() for proper GPU setup via CUDA_VISIBLE_DEVICES
2. Each rank gets a unique GPU through proper device isolation (handled by distribution_lib)
3. Simplified code - removed duplicate GPU setup functions
"""
...
def run_opt_hybrid_dp_mp_test():
    ...
    print(f"CRITICAL FIX: GPU isolation via CUDA_VISIBLE_DEVICES in distribution_lib")
    print(f"{'='*70}")
    print(f"Local rank: {local_rank}, World size: {world_size}, Backend: {backend}")

    # V15 FIX: GPU setup is now handled by distribution_lib.initialize()
    # Just verify CUDA is working after distribution_lib sets it up
    if torch.cuda.is_available():
        current_device = torch.cuda.current_device()
        print(f"[Rank {local_rank}] Current CUDA device after init: {current_device}")
    ...
    # Initialize distributed backend (V15: handles GPU setup via CUDA_VISIBLE_DEVICES)
