...
FIXED VERSION V10: Fixes regex patterns and DTensor conversion issues.

KEY FIXES:
1. Fixed regex patterns to match actual weight names (feedforward vs feed_forward)
2. Fixed _layout_to_placements to shard on correct tensor dimension
3. Uses proper DTensor conversion via torch_distribute_tensor
4. Ensures all ranks synchronize before DTensor operations
...
def redistribute_model_weights_properly(model, strategy, layout_map):
    ...
    for v in model.trainable_variables:
        ...
        for pattern, layout in layout_dict.items():
            # Create a simple pattern without regex special chars for contains check
            simple_pattern = pattern.replace('.*', '').replace('\\/', '/')
            
            # Check if pattern matches the variable path
            if simple_pattern in v.path:
        ...
        if needs_shard:
            try:
                ...
                # This handles the sharding correctly
...
def run_opt_hybrid_dp_mp_test():
    ...
    print(f"\n{'='*70}")
    print(f"TEST: OPT MODEL WITH HYBRID DATA PARALLEL + MODEL PARALLEL (V10)")
    print(f"{'='*70}")
    ...
    # Create LayoutMap for OPT with CORRECTED patterns
    # The OPT model uses "feedforward" (no underscore), not "feed_forward"
    layout_map = LayoutMap(mesh)

    # CRITICAL: Fix the patterns to match the actual weight names
    # OPT uses: transformer_layer_X/feedforward_intermediate_dense/kernel
    # NOT: transformer_layer_X/feed_forward_intermediate_dense/kernel
    layout_map[".*feedforward.*kernel"] = (None, "model")  # Column Parallelism - fixed pattern
    layout_map[".*feedforward.*bias"] = ()  # Replicated bias
    ...
    print(f"[Rank {local_rank}] NOTE: Using '.*feedforward.*' pattern (no underscore)")
