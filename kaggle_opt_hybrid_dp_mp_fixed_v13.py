...
FIXED VERSION V13: Fix regex pattern matching (re.match -> re.search).

KEY FIXES:
1. CRITICAL: Use re.search() instead of re.match() for pattern matching
   ...
   - Patterns need to search anywhere in the path, not just at start
2. Convert input tensors to DTensors before forward pass when distribution is enabled
...
def prepare_input_for_distribution(x, device_mesh=None):
    ...
    if device_mesh is None:
        # Try to get from distribution
        dist = get_distribution()
        if dist is not None:
            device_mesh = global_state.get_global_attribute("torch_device_mesh", None)
    
    if device_mesh is None:
        # Create a simple mesh for input distribution
    ...
    try:
        # Replicate the input across all mesh dimensions
        # For data parallel, we typically replicate along the batch dimension
        # For model parallel, we replicate all inputs
...
def redistribute_model_weights_properly(model, strategy, layout_map):
    ...
    Variable paths look like: "transformer_layer_0/feedforward_intermediate_dense/kernel"
    We need to search anywhere in the path, not just at start.
    """
    import re
    from torch.distributed._tensor import DTensor, Replicate, Shard
    from torch.distributed.device_mesh import init_device_mesh
                ...
                if hasattr(v, '_value') and v._value is not None:
                    ...
                    class DTensorParameter(torch.nn.Parameter):
                        ...
                        def __init__(self, dtensor):
                            # Detach from graph, we don't need gradients for the wrapper
                    ...
                    # Create wrapper parameter
                    wrapper = DTensorParameter(dtensor)
                    
                    # Replace the internal value
...
def run_opt_hybrid_dp_mp_test():
    ...
    print(f"\n{'='*70}")
    print(f"TEST: OPT MODEL WITH HYBRID DATA PARALLEL + MODEL PARALLEL (V13)")
    print(f"CRITICAL FIX: re.search() instead of re.match() for regex matching")
    print(f"{'='*70}")
    ...
    # This prevents "Duplicate GPU detected" NCCL errors
    ...
    # Create LayoutMap for OPT with FIXED patterns
    layout_map = LayoutMap(mesh)

    # FIXED PATTERNS V13: Use proper regex patterns that match the actual weight names
    # The actual weight paths look like:
    # - transformer_layer_0/self_attention/query/kernel
    # - transformer_layer_0/feedforward_intermediate_dense/kernel
    
    # CRITICAL V13 FIX: Patterns use .* which means "match anything"
    # These patterns will match anywhere in the path because we use re.search()
    layout_map[".*feedforward.*intermediate.*dense.*kernel"] = (None, "model")  # Column Parallelism
    layout_map[".*feedforward.*output.*dense.*kernel"] = (None, "model")  # Column Parallelism
    layout_map[".*feedforward.*intermediate.*dense.*bias"] = ()  # Replicated bias
    layout_map[".*feedforward.*output.*dense.*bias"] = ()  # Replicated bias
    
    # Shard attention projections on dim 1 (input features)
    # The self_attention layer has query, key, value, attention_output
    ...
    layout_map[".*self_attention.*layer_norm.*gamma"] = ()
    layout_map[".*self_attention.*layer_norm.*beta"] = ()
    layout_map[".*feedforward.*layer_norm.*gamma"] = ()
    layout_map[".*feedforward.*layer_norm.*beta"] = ()
    layout_map[".*layer_norm.*gamma"] = ()
    layout_map[".*layer_norm.*beta"] = ()
    ...
    print(f"[Rank {local_rank}] CRITICAL: Using re.search() for pattern matching (not re.match)")
    ...
    try:
        ...
        if is_opt_model:
            ...
            # CRITICAL V13 FIX: Convert input tensors to DTensors before forward pass
            # This prevents "mixed torch.Tensor and DTensor" errors
            ...
            with torch.no_grad():
                # Build the input dictionary in the format Keras expects
                input_dict = {
            ...
            if isinstance(model.layers[-1], keras.layers.Dense) and model.output_shape[-1] == 10:
                ...
                # CRITICAL V13 FIX: Convert input to DTensor
                x_tensor = torch.from_numpy(x)
                if torch.cuda.is_available():
                    x_tensor = x_tensor.cuda()
    ...
    try:
        ...
        if is_opt_model:
            ...
            # CRITICAL V13 FIX: Convert training inputs to DTensors
            token_ids_tensor = torch.from_numpy(token_ids).long()
            padding_mask_tensor = torch.from_numpy(padding_mask)
