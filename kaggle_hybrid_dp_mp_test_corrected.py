...
This is a corrected version that properly detects DTensor weights.
"""
...
def verify_sharding(model, rank):
    ...
    This function properly detects DTensor weights by checking if the 
    underlying tensor is a PyTorch DTensor.
    """
    ...
    for i, layer in enumerate(model.layers):
        ...
        if hasattr(layer, 'weights'):
            for weight in layer.weights:
                ...
                # Check if the underlying tensor is a DTensor
                # For torch.nn.Parameter wrapping DTensor, check the _torch attribute
                # The weight itself is a torch.nn.Parameter wrapping a DTensor
                weight_tensor = weight.data if hasattr(weight, 'data') else weight
                
                if isinstance(weight_tensor, DTensor):
                    # Direct DTensor check
                    ...
                    print(f"[Rank {rank}]   - Placements: {placements}")
